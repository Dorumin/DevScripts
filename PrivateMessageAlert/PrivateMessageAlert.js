...
(function reloadUntilMainRoom() {
    if (
        ...
        (window.PrivateMessageAlert && PrivateMessageAlert.init)
    ) return;
    ...
    if (typeof dev == 'undefined' || typeof dev.i18n == 'undefined') { // Import i18n-js, if needed
        importArticle({
            type: 'script',
    ...
    window.PrivateMessageAlert = $.extend({
        ...
        onMessage: function(model) {
            ...
            room = model.attributes.roomId,
            icon = model.attributes.avatarSrc.replace(/28(?!.*28)/, '150');
            this.alert(name, text, icon, room);
        },
        init: function(i18n) {
            this.i18n = i18n;
            ...
            mainRoom.model.privateUsers.bind('add', $.proxy(function(user) {
                ...
                room.model.chats.bind('afteradd', $.proxy(this.onMessage, this));
            }, this));
