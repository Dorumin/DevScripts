...
(function() {
    ...
    var QuickLogs = {
        ...
        isAdmin: function() {
            return this.hasRights(['bureaucrat', 'sysop', 'helper', 'vstf', 'staff', 'global-discussions-moderator']);
        },
        ...
        isChatMod: function() {
            return this.chat &&
                   this.hasRights(['chatmoderator', 'threadmoderator', 'sysop', 'helper', 'staff', 'vstf']);
        },
        ...
        makeLink: function(href, text, striked) {
            return {
                type: 'a',
        ...
        parseRights: function(rights) {
            ...
            return {
                oldstr: oldarr.map(function(group) {
                    if (group && newarr.indexOf(group) == -1) return '<b>' + group + '</b>';
                    return group;
                }).join(', ') || this.i18n.msg('none').escape(),
                newstr: newarr.map(function(group) {
                    if (group && oldarr.indexOf(group) == -1) return '<b>' + group + '</b>';
                    return group;
                }).join(', ') || this.i18n.msg('none').escape()
            };
        },
        ...
        render: function() {
            ...
            while (logType = this.logOrder[i++]) {
                ...
                if (!log.check || log.check()) {
                    if (log.href) {
                        children.push(this.makeLink(log.href, log.message || this.i18n.msg(log.key).plain(), log.striked));
                    } else {
        ...
        addLink: function(key, obj, index) {
            ...
            this.logOrder.splice(index === undefined ? this.logOrder.length - 1 : index, 0, key);
            if (this.built) {
                this.render();
            }
            return true;
