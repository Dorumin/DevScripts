...
(function() {
    if (wgNamespaceNumber != 1201 || window.ReplyListInit) return;
    ...
    if (window.ReplyList && ReplyList.noStyle) return;
    // Add CSS to make it extra sexy
    mw.util.addCSS('.reply-posts {                          \
        text-align: right;                                  \
        margin-bottom: 2px;                                 \
    }                                                       \
    .reply-posts a {                                        \
    ...
    #Wall .mouse-highlight > .speech-bubble-message {       \
        box-shadow: 0 0 5px rgba(81, 203, 238, 1);          \
    }                                                       \
    .replies .mouse-highlight > .speech-bubble-message {    \
        padding-top: 5px;                                   \
    }                                                       \
    .thread-tooltip .wikia-menu-button,                     \
    .thread-tooltip .follow,                                \
    .thread-tooltip .reply-posts,                           \
    .thread-tooltip .msg-title {                            \
        display: none;                                      \
    }                                                       \
    .thread-tooltip .avatar {                               \
        height: 30px;                                       \
        width: 30px;                                        \
    }');
})();