...
mw.loader.using(['mediawiki.api', 'jquery.ui.autocomplete']).then(function() {
    if (wgCanonicalSpecialPageName != 'Disambiguations' || !$('.special').length || $('#dm-button').length) return;
    ...
    function get_all_results(index, callback, merge) {
        ...
        Api.get({
            ...
            Api.get({
                ...
                var pages = Object.keys(p.query.pages).map(function(key) {
    ...
    function display_editor(title, content, matches) {
        ...
        i18n.useContentLang(); // Switch to content lang for summary
        update_state($('<input>', {
            id: 'dm-summary-input',
            val: summary || '',
            keyup: function() {
        ...
        }).attr('placeholder', i18n.msg('default-summary').plain()));
        i18n.useUserLang();
    }
    ...
    importArticles({
        type: 'style',
        articles: [
            'u:dev:MediaWiki:DisambiguationManagement/code.css'
        ]
    });
