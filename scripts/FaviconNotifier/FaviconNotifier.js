...
(function reloadUntilMainRoom() {
    ...
    if (window.FaviconNotifierINIT || wgCanonicalSpecialPageName != 'Chat') return;
    ...
    FaviconNotifier.canvas = (function() {
        ...
        img.src = cors_api_url + FaviconNotifier.icon;
        //console.log(img.src);
        img.onload = function () {
            canvas.width = 32;
            canvas.height = 32;
        ...
        img.onerror = function () {
            console.log('The site that allows cross-origin loading is down. FaviconNotifier will not run.');
        };
        return canvas;
    })();
    ...
    mw.hook('FaviconNotifier.loaded').add(function(canvas) {
        var afteradd = function(chat) {
            if (document.hasFocus() || chat.attributes.name == wgUserName || (chat.attributes.isInlineAlert && !FaviconNotifier.includeInlineAlerts)) return;
