 ...
 *
 * Add translations here: https://dev.wikia.com/wiki/Special:BlankPage/I18nEdit/ViewDeleted
 *
 * @scope Personal
 * @author Dorumin
 */
...
mw.loader.using('mediawiki.api').then(function() {
    ...
    can_restore = (ug.indexOf('sysop') + ug.indexOf('content-moderator') + ug.indexOf('vstf') + ug.indexOf('staff') + ug.indexOf('helper') != -5),
    config = window.ViewDeleted || {rc: true, logs: 'replace'},
    rc = config.rc && $('#recentchanges-options + .rc-conntent').length,
    logs = config.logs && (wgPageName.slice(-7) == '/delete' || $.getUrlVar('type') == 'delete');
    config.modal = config.modal || {};
    config.modal.preview = config.modal.preview || {};
    config.modal.content = config.modal.content || {};
    if (window.ViewDeleted_init || wgNamespaceNumber != -1 || !(!rc || !logs) || !can_restore) return;
    ...
    function add_links() {
        ...
        if ((wgPageName.slice(-7) == '/delete' || $.getUrlVar('type') == 'delete') && config.logs) { // Special:Log/delete
            ...
            if ($.getUrlVar('hideenhanced') == 1) {
                var $elems = $('.special > li').filter(function() {
                    ...
                    prefix = '/wiki/' + Object.keys(wgNamespaceIds)[1];
    ...
    function show_modal(e) {
        ...
        file_ns = Object.keys(wgNamespaceIds).filter(function(el) {
            return wgNamespaceIds[el] == 6;
        }),
    ...
    function hacky_image_preview(title, callback) {
        $.get('/wiki/Special:Undelete?target=' + encodeURIComponent(title), function(page) {
            var $page = $(page),
