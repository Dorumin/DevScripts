/* Pings
 *
 * Yet another ping script for chat
 * But there's one thing that makes this one different and better than the rest:
 * @author Dorumin
 */
...
(function() {
    ...
    window.Pings = $.extend({
                            ...
                            children: [
                                {
                                    ...
                                    style: {
                                        color: option == 'color' ? this.storage.color : ''
                                    },
        ...
        showInvalidSettingsModal: function(error) {
            this.$invalidSettingsModal = $.showCustomModal(
                this.i18n.msg('invalid-settings-modal-header').escape(),
                this.i18n.msg('invalid-settings-modal-content', error).escape(),
                {
                    ...
                    buttons: [
                        {
                            ...
                            defaultButton: true,
                            message: this.i18n.msg('modal-button-close').escape(),
                            handler: this.closeInvalidSettingsModal.bind(this)
                        }
                    ]
                }
            );
        },
        closeInvalidSettingsModal: function() {
            if (!this.$invalidSettingsModal) return;
            this.$invalidSettingsModal.closeModal();
            this.$invalidSettingsModal = null;
        },
        playAudio: function() {
        ...
        showModal: function() {
            this.$modal = $.showCustomModal(this.i18n.msg('modal-header').escape(),
                dev.ui({
                    type: 'div',
                    attr: {
                        id: 'PingsContainer'
                ...
                {
                    id: 'PingsModal',
                    width: Math.max(420, Math.min(500, innerWidth - 200)),
                    buttons: [
                        {
                            id: 'PingsModalCancelButton',
                            message: this.i18n.msg('modal-button-cancel').escape(),
                            handler: this.closeModal.bind(this)
                        },
                        {
                            id: 'PingsModalSaveButton',
                            defaultButton: true,
                            message: this.i18n.msg('modal-button-save').escape(),
                            handler: this.modalSave.bind(this)
                        }
                    ]
                }
            );
