 ...
 * @maintainer: Thundercraft5 (https://dev.fandom.com/wiki/User:Thundercraft5)
 * @scope: Personal use
 ...
 * @update: 20/10/2020: Adding UCP support, some new features, as well as general improvements.
 */
...
$.when(
    ...
    function build_checkbox(id, label, checked, $after) {
        var $wrapper = $('<div>'),
            $check = $('<input>')
                .attr('type', 'checkbox')
                .attr('id', id)
                ...
                handler: function() {
                    ...
                    Api.post(config).always(function(d) {
                        ...
                        if (d.unblock) {
                            ...
                            console.log(i18n.msg('success-unblock', user).escape());
                        } else {
                            !isUCP
                            ...
                            console.warn(i18n.msg('error-unblock', user, d).escape());
                        }
                    });
                }
            }, {
                ...
                handler: function() {
                    ...
                    Api.post(query).always(function(d) {
                        ...
                        if (d.block) {
                            ...
                            console.log(i18n.msg('success-block', user).escape())

                            if (rangeBlock && isIP && config.extras.rangeblock) {
                                Api.post(rangeBlockQuery)
                                    .always(function(d) {
                                        var errorMsg = 'API error in blocking the ' + range + ' CIDR range for "' + user + '": ' + (isUCP ? d : d.error.info)+ ' (API Error Code "' + (isUCP ? d : d.error.code) + '")',
                                            successMsg = 'The ' + range + ' CIDR range for "' + user + '" has been blocked sucessfully!'
                                        if (d.block) {
                                            !isUCP
                                                ? new BannerNotification(successMsg, 'confirm', $('.banner-notifications-placeholder')).show()
                                                : mw.notify(successMsg, { type: "success" })
                                            console.log(successMsg)
                                        } else {
                                            !isUCP
                                                ? new BannerNotification(errorMsg, 'warn', $('.banner-notifications-placeholder')).show()
                                                : mw.notify(errorMsg, { type: "warn" })
                            ...
                            !isUCP
                                ? new BannerNotification(i18n.msg('error-block', user, d.error.info).escape(), 'warn', $('.banner-notifications-placeholder')).show()
                                : mw.notify(i18n.msg('error-block', user, d).escape(), { type: "warn" })
                            console.warn(i18n.msg('error-block', user, isUCP ? d : d.error.info).escape())
                        }
                    });
                }
            }, {
